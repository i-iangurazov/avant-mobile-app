...
rows=list(csv.DictReader((ROOT/'CRITERIA.csv').open()))
...
(ROOT/'SCORES.json').write_text(json.dumps(result,ensure_ascii=False,indent=2))
...
for p,s in result.items():
 ...
 lines.append('')
(ROOT/'CALCULATION.md').write_text('\n'.join(lines))
