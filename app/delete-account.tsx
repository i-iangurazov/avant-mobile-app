...
export default function DeleteAccount(){const {user,session,signOut}=useAuth();const docs=useDocuments();
 ...
 const submit=async()=>{setLoading(true);setError('');try{
  await appApiClient.request('/profile',{method:'DELETE',headers:{Authorization:`Bearer ${session?.accessToken}`},body:JSON.stringify({password,phoneProof:proof})});
 ...
 <Pressable accessibilityRole="checkbox" accessibilityState={{checked:confirmed}} onPress={()=>setConfirmed(!confirmed)} style={{minHeight:48,justifyContent:'center'}}><Text>{confirmed?'☑':'☐'} Я прочитал порядок удаления и подтверждаю удаление аккаунта и связанных данных.</Text></Pressable>
