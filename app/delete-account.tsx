...
export default function DeleteAccount(){const {user,session,signOut}=useAuth();const docs=useDocuments();
 const [password,setPassword]=useState('');const [confirmed,setConfirmed]=useState(false);const [error,setError]=useState('');const [loading,setLoading]=useState(false);
 ...
 const submit=async()=>{setLoading(true);setError('');try{
  await appApiClient.request('/profile',{method:'DELETE',headers:{Authorization:`Bearer ${session?.accessToken}`},body:JSON.stringify({password,deletionDocumentVersion:policy?.version})});
 ...
 <AppButton title="Удалить аккаунт и данные" variant="danger" disabled={!policy||!confirmed||!password} loading={loading} onPress={()=>void submit()} /></>}
