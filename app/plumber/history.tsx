...
export default function LoyaltyHistoryScreen() {
  ...
  return (
    <SafeAreaView edges={["top"]} style={styles.safe}>
      ...
      <View style={styles.filters}>{filters.map((filter) => <Pressable accessibilityRole="button" key={filter.label} onPress={() => setStatus(filter.value)} style={[styles.filter, status === filter.value && styles.filterActive]}><Text style={[styles.filterText, status === filter.value && styles.filterTextActive]}>{filter.label}</Text></Pressable>)}</View>
      ...
      {!history.isLoading && !history.isError && !history.data?.length ? <EmptyState title="Операций пока нет" text="Начисления появятся после обработки чека с вашим QR-кодом." icon="receipt-outline" /> : null}
      {history.data?.length ? <FlatList data={history.data} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} onRefresh={() => void history.refetch()} refreshing={history.isRefetching} renderItem={({ item }) => {
