...
export default function AdminOrderScreen() {
  ...
  return <SafeAreaView edges={['top', 'bottom']} style={styles.safe}>
    ...
    {!user?.isAdmin ? <EmptyState title="Доступно только администратору" /> : <ScrollView contentContainerStyle={styles.content}>
      ...
      {value ? <>
        <View style={styles.card}><StatusBadge status={value.status} /><Text style={styles.title}>{formatPrice(value.total_amount, value.total_label ?? 'Уточняется менеджером')}</Text>{value.availability_notice ? <Text style={styles.text}>{value.availability_notice}</Text> : null}<Text style={styles.text}>{formatDate(value.created_at)}</Text><Text style={styles.text}>{telegramDeliveryLabel(value.notificationStatus)}</Text></View>
