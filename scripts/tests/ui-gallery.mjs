...
const sessions=JSON.parse(readFileSync('/private/tmp/avantehnik-remediation-sessions.json'));const base='http://127.0.0.1:8090';const port=9360;
const chrome=spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',['--headless=new','--disable-gpu','--no-first-run','--disable-background-networking','--disable-sync','--remote-debugging-port='+port,'--user-data-dir=/private/tmp/remediation-gallery-chrome','about:blank'],{stdio:'ignore'});
...
try{
 ...
 await role('guest');
 for(const p of ['/welcome','/choice','/login','/register','/catalog','/category/all-products','/product/'+sessions.productId,'/cart','/orders','/profile','/maps','/image-search','/profile/about','/legal/privacy','/legal/loyalty','/delete-account','/password-reset','/find-plumber','/plumber/apply','/admin']){await nav(p);await capture('guest-'+p.slice(1).replaceAll('/','-'),'guest',p);}
 await nav('/register');await click('Создать аккаунт');await capture('register-validation','guest','/register','invalid empty');
 await role('customer');
 for(const p of ['/profile','/delete-account','/profile/edit','/profile/telegram','/orders','/orders/'+sessions.orderId,'/find-plumber','/plumber/apply']){await nav(p);await capture('customer-'+p.slice(1).replaceAll('/','-'),'customer',p);}
 await nav('/category/all-products');await click('В корзину');await wait(300);await nav('/cart');await capture('customer-cart-populated','customer','/cart');
 await nav('/checkout');await capture('customer-checkout-pickup','customer','/checkout');await nav('/checkout');await ev(`Array.from(document.querySelectorAll('[role=radio]')).find(x=>x.innerText.trim()==='Доставка')?.click()`);await capture('customer-checkout-delivery','customer','/checkout','delivery');
 await role('plumber');
